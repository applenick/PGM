...
public class UserCommands {
  ...
  public void list(CommandSender sender, MatchPlayer player, Match match) {
    ...
    if (tmm != null) {
      sender.sendMessage(
          new PersonalizedTranslatable("commands.users.teams")
              .getPersonalizedText()
      ...
      sendTeamInfo(
          sender,
          new PersonalizedTranslatable("commands.users.participants"),
  ...
  private Component formatNames(Collection<MatchPlayer> players, CommandSender sender) {
    List<Component> names =
        players.stream()
            ...
            .map(mp -> new Component(mp.getStyledName(NameStyle.FANCY).render(sender)))
